...
class SDKServer {
  ...
  private getEffectiveAppId() {
    ...
    return appId && appId.length > 0 ? appId : "idlr-pts-platform";
  }
  ...
  async signSession(
    ...
    const effectiveAppId = payload.appId?.trim() || this.getEffectiveAppId();
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    const previewRoleMatch = /^preview-(user|surveyor|registrar|admin)(?:-|$)/.exec(sessionUserId);
    let user: User | undefined;

    try {
      user = await db.getUserByOpenId(sessionUserId);
    } catch (error) {
      console.warn("[Auth] Database lookup failed during session authentication", error);
      user = undefined;
    }
    ...
    if (!user && previewRoleMatch && process.env.NODE_ENV !== "production") {
      ...
      return {
        ...
        name: session.name || `Preview ${previewRole}`,
    ...
    try {
      ...
      console.warn("[Auth] Unable to persist lastSignedIn during session authentication", error);
    }

...
export function batchAssignParcels(parcelIds: number[], surveyorId: string) {
  const store = loadStore();
  const now = new Date().toISOString();
  const updated: ParcelRecord[] = [];

  for (const parcel of store.parcels) {
    if (!parcelIds.includes(parcel.id)) continue;
    parcel.surveyorId = surveyorId;
    parcel.updatedAt = now;
    updated.push(parcel);
  }

  if (updated.length === 0) {
    throw new Error('No parcels found for assignment');
  }
...
export function batchVerifyParcels(parcelIds: number[], verifierId: string) {
  const store = loadStore();
  const now = new Date().toISOString();
  const updated: ParcelRecord[] = [];
  ...
  for (const parcel of store.parcels) {
    if (!parcelIds.includes(parcel.id)) continue;
    if (parcel.status === 'registered') continue;
    parcel.status = 'verified';
    parcel.verifierId = verifierId;
  ...
  if (updated.length === 0) {
    throw new Error('No parcels found for verification');
  }

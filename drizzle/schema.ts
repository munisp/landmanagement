...
export const commercialMemberRoleEnum = pgEnum("commercial_member_role", ["owner", "billing_admin", "lender_admin", "lender_analyst", "reviewer", "matter_manager", "legal_reviewer", "field_manager", "field_inspector", "field_reviewer", "registry_admin", "registry_supervisor", "registry_officer", "row_manager", "row_officer", "row_reviewer", "tax_manager", "tax_assessor", "tax_reviewer"]);
...
export const registryOperationCaseStatus = pgEnum("registry_operation_case_status", ["submitted", "triaged", "in_review", "returned", "completed", "withdrawn"]);

export const registryOperationQueues = pgTable("registry_operation_queues", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(),
  accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }),
  queueKey: varchar("queue_key", { length: 96 }).notNull().unique(),
  name: varchar("name", { length: 160 }).notNull(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ accountEnabled: index("registry_operation_queues_account_enabled_idx").on(table.accountId, table.enabled) }));

export const registryOperationCases = pgTable("registry_operation_cases", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(),
  caseKey: varchar("case_key", { length: 96 }).notNull().unique(),
  accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id),
  queueId: bigint("queue_id", { mode: "number" }).notNull().references(() => registryOperationQueues.id),
  parcelId: integer("parcel_id").references(() => parcels.id),
  requestReference: varchar("request_reference", { length: 160 }).notNull(),
  requesterName: varchar("requester_name", { length: 255 }),
  requesterContactReference: varchar("requester_contact_reference", { length: 160 }),
  status: registryOperationCaseStatus("status").notNull().default("submitted"),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ accountStatusDue: index("registry_operation_cases_account_status_due_idx").on(table.accountId, table.status, table.dueAt), queueAssignee: index("registry_operation_cases_queue_assignee_idx").on(table.queueId, table.assignedTo, table.status) }));

export const registryOperationEvents = pgTable("registry_operation_events", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(),
  caseId: bigint("case_id", { mode: "number" }).notNull().references(() => registryOperationCases.id, { onDelete: "cascade" }),
  eventType: varchar("event_type", { length: 64 }).notNull(),
  previousStatus: registryOperationCaseStatus("previous_status"),
  nextStatus: registryOperationCaseStatus("next_status"),
  actorId: integer("actor_id").notNull().references(() => users.id),
  description: text("description").notNull(),
  metadata: jsonb("metadata").notNull().default({}),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ caseCreated: index("registry_operation_events_case_created_idx").on(table.caseId, table.createdAt) }));


export const rowAgreementStatusEnum = pgEnum("row_agreement_status", ["draft", "proposed", "under_review", "executed", "expired", "terminated"]);
export const rowFindingStatusEnum = pgEnum("row_finding_status", ["identified", "verified", "resolved", "dismissed"]);
export const rowCorridors = pgTable("row_corridors", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), corridorKey: varchar("corridor_key", { length: 96 }).notNull().unique(), name: varchar("name", { length: 200 }).notNull(), purpose: varchar("purpose", { length: 96 }).notNull(), geometryGeojson: jsonb("geometry_geojson").notNull(), createdBy: integer("created_by").notNull().references(() => users.id), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ accountIdx: index("row_corridors_account_idx").on(table.accountId) }));
export const rowParcelFindings = pgTable("row_parcel_findings", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), findingKey: varchar("finding_key", { length: 96 }).notNull().unique(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), corridorId: bigint("corridor_id", { mode: "number" }).notNull().references(() => rowCorridors.id, { onDelete: "cascade" }), parcelId: integer("parcel_id").notNull().references(() => parcels.id), status: rowFindingStatusEnum("status").notNull().default("identified"), overlapMethod: varchar("overlap_method", { length: 64 }).notNull(), overlapSummary: text("overlap_summary").notNull(), sourceReference: varchar("source_reference", { length: 160 }).notNull(), reviewedBy: integer("reviewed_by").references(() => users.id), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ accountStatus: index("row_findings_account_status_idx").on(table.accountId, table.status) }));
export const rowAccessAgreements = pgTable("row_access_agreements", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), agreementKey: varchar("agreement_key", { length: 96 }).notNull().unique(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), findingId: bigint("finding_id", { mode: "number" }).notNull().references(() => rowParcelFindings.id), status: rowAgreementStatusEnum("status").notNull().default("draft"), agreementReference: varchar("agreement_reference", { length: 160 }).notNull(), effectiveOn: date("effective_on"), expiresOn: date("expires_on"), termsReference: varchar("terms_reference", { length: 160 }).notNull(), createdBy: integer("created_by").notNull().references(() => users.id), approvedBy: integer("approved_by").references(() => users.id), approvedAt: timestamp("approved_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ accountStatusExpiry: index("row_agreements_account_status_expiry_idx").on(table.accountId, table.status, table.expiresOn) }));
export const rowFieldConfirmations = pgTable("row_field_confirmations", {
  id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), confirmationKey: varchar("confirmation_key", { length: 96 }).notNull().unique(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), findingId: bigint("finding_id", { mode: "number" }).notNull().references(() => rowParcelFindings.id), latitude: doublePrecision("latitude").notNull(), longitude: doublePrecision("longitude").notNull(), observedAt: timestamp("observed_at", { withTimezone: true }).notNull(), evidenceReference: varchar("evidence_reference", { length: 160 }).notNull(), note: text("note").notNull(), capturedBy: integer("captured_by").notNull().references(() => users.id), reviewedBy: integer("reviewed_by").references(() => users.id), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({ findingIdx: index("row_field_confirmations_finding_idx").on(table.findingId, table.observedAt) }));
export const rowEvents = pgTable("row_events", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), entityType: varchar("entity_type", { length: 64 }).notNull(), entityKey: varchar("entity_key", { length: 96 }).notNull(), eventType: varchar("event_type", { length: 64 }).notNull(), actorId: integer("actor_id").notNull().references(() => users.id), description: text("description").notNull(), metadata: jsonb("metadata").notNull().default({}), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() }, (table) => ({ accountEntity: index("row_events_account_entity_idx").on(table.accountId, table.entityKey, table.createdAt) }));


export const taxAssessmentStatusEnum = pgEnum("tax_assessment_status", ["draft", "evidence_pending", "under_review", "issued", "appealed", "closed", "withdrawn"]);
export const taxAppealStatusEnum = pgEnum("tax_appeal_status", ["filed", "under_review", "upheld", "adjusted", "withdrawn"]);
export const taxAssessmentCases = pgTable("tax_assessment_cases", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), caseKey: varchar("case_key", { length: 96 }).notNull().unique(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), parcelId: integer("parcel_id").notNull().references(() => parcels.id), taxYear: integer("tax_year").notNull(), status: taxAssessmentStatusEnum("status").notNull().default("draft"), declaredBasisMinor: bigint("declared_basis_minor", { mode: "number" }), currency: varchar("currency", { length: 3 }).notNull().default("USD"), assessedAmountMinor: bigint("assessed_amount_minor", { mode: "number" }), assessmentReference: varchar("assessment_reference", { length: 160 }), createdBy: integer("created_by").notNull().references(() => users.id), reviewedBy: integer("reviewed_by").references(() => users.id), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), issuedAt: timestamp("issued_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => ({ accountStatus: index("tax_cases_account_status_idx").on(table.accountId, table.status, table.updatedAt) }));
export const taxAssessmentEvidence = pgTable("tax_assessment_evidence", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), evidenceKey: varchar("evidence_key", { length: 96 }).notNull().unique(), caseId: bigint("case_id", { mode: "number" }).notNull().references(() => taxAssessmentCases.id, { onDelete: "cascade" }), kind: varchar("kind", { length: 64 }).notNull(), sourceReference: varchar("source_reference", { length: 160 }).notNull(), description: text("description").notNull(), submittedBy: integer("submitted_by").notNull().references(() => users.id), reviewedBy: integer("reviewed_by").references(() => users.id), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), accepted: boolean("accepted"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() }, (table) => ({ caseIdx: index("tax_evidence_case_idx").on(table.caseId, table.createdAt) }));
export const taxAssessmentAppeals = pgTable("tax_assessment_appeals", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), appealKey: varchar("appeal_key", { length: 96 }).notNull().unique(), caseId: bigint("case_id", { mode: "number" }).notNull().references(() => taxAssessmentCases.id, { onDelete: "cascade" }), status: taxAppealStatusEnum("status").notNull().default("filed"), grounds: text("grounds").notNull(), evidenceReference: varchar("evidence_reference", { length: 160 }).notNull(), filedBy: integer("filed_by").notNull().references(() => users.id), decidedBy: integer("decided_by").references(() => users.id), decisionNote: text("decision_note"), decidedAt: timestamp("decided_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => ({ caseIdx: index("tax_appeals_case_idx").on(table.caseId, table.status) }));
export const taxPaymentReferences = pgTable("tax_payment_references", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), paymentKey: varchar("payment_key", { length: 96 }).notNull().unique(), caseId: bigint("case_id", { mode: "number" }).notNull().references(() => taxAssessmentCases.id), provider: varchar("provider", { length: 32 }).notNull(), providerReference: varchar("provider_reference", { length: 160 }).notNull(), amountMinor: bigint("amount_minor", { mode: "number" }).notNull(), currency: varchar("currency", { length: 3 }).notNull(), verifiedAt: timestamp("verified_at", { withTimezone: true }), verifiedBy: integer("verified_by").references(() => users.id), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const taxOperationEvents = pgTable("tax_operation_events", { id: bigint("id", { mode: "number" }).primaryKey().generatedAlwaysAsIdentity(), accountId: bigint("account_id", { mode: "number" }).notNull().references(() => commercialAccounts.id, { onDelete: "cascade" }), caseKey: varchar("case_key", { length: 96 }).notNull(), eventType: varchar("event_type", { length: 64 }).notNull(), actorId: integer("actor_id").notNull().references(() => users.id), description: text("description").notNull(), metadata: jsonb("metadata").notNull().default({}), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });


export const portfolioReviewStatusEnum = pgEnum("portfolio_review_status", ["draft", "open", "under_review", "approved", "closed", "withdrawn"]);
export const providerVerificationStatusEnum = pgEnum("provider_verification_status", ["pending", "verified", "rejected", "suspended"]);
export const serviceRequestStatusEnum = pgEnum("service_request_status", ["submitted", "accepted", "in_progress", "completed", "cancelled", "disputed"]);
export const acquisitionDatarooms = pgTable("acquisition_datarooms", { id: bigint("id", { mode:"number" }).primaryKey().generatedAlwaysAsIdentity(), accountId: bigint("account_id", {mode:"number"}).notNull().references(()=>commercialAccounts.id,{onDelete:"cascade"}), dataroomKey:varchar("dataroom_key",{length:96}).notNull().unique(), name:varchar("name",{length:200}).notNull(), purpose:varchar("purpose",{length:160}).notNull(), status:portfolioReviewStatusEnum("status").notNull().default("draft"), createdBy:integer("created_by").notNull().references(()=>users.id), createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(), updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow() },(t)=>({accountIdx:index("acquisition_dataroom_account_idx").on(t.accountId,t.status)}));
export const acquisitionDueDiligenceItems = pgTable("acquisition_due_diligence_items", { id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(), itemKey:varchar("item_key",{length:96}).notNull().unique(), dataroomId:bigint("dataroom_id",{mode:"number"}).notNull().references(()=>acquisitionDatarooms.id,{onDelete:"cascade"}), parcelId:integer("parcel_id").references(()=>parcels.id), category:varchar("category",{length:64}).notNull(), sourceReference:varchar("source_reference",{length:160}).notNull(), factualSummary:text("factual_summary").notNull(), status:portfolioReviewStatusEnum("status").notNull().default("open"), reviewedBy:integer("reviewed_by").references(()=>users.id), reviewedAt:timestamp("reviewed_at",{withTimezone:true}), createdBy:integer("created_by").notNull().references(()=>users.id), createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(), updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow() });
export const exposurePortfolios=pgTable("exposure_portfolios",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),accountId:bigint("account_id",{mode:"number"}).notNull().references(()=>commercialAccounts.id,{onDelete:"cascade"}),portfolioKey:varchar("portfolio_key",{length:96}).notNull().unique(),name:varchar("name",{length:200}).notNull(),status:portfolioReviewStatusEnum("status").notNull().default("open"),createdBy:integer("created_by").notNull().references(()=>users.id),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow()},(t)=>({accountIdx:index("exposure_portfolio_account_idx").on(t.accountId,t.status)}));
export const exposureAssets=pgTable("exposure_assets",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),portfolioId:bigint("portfolio_id",{mode:"number"}).notNull().references(()=>exposurePortfolios.id,{onDelete:"cascade"}),assetKey:varchar("asset_key",{length:96}).notNull().unique(),parcelId:integer("parcel_id").references(()=>parcels.id),label:varchar("label",{length:200}).notNull(),latitude:doublePrecision("latitude").notNull(),longitude:doublePrecision("longitude").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()});
export const exposureSnapshots=pgTable("exposure_snapshots",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),snapshotKey:varchar("snapshot_key",{length:96}).notNull().unique(),portfolioId:bigint("portfolio_id",{mode:"number"}).notNull().references(()=>exposurePortfolios.id,{onDelete:"cascade"}),contextLayer:varchar("context_layer",{length:64}).notNull(),sourceReference:varchar("source_reference",{length:160}).notNull(),observedAt:timestamp("observed_at",{withTimezone:true}).notNull(),summary:text("summary").notNull(),humanReviewNote:text("human_review_note"),createdBy:integer("created_by").notNull().references(()=>users.id),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()});
export const propertyApiClients=pgTable("property_api_clients",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),accountId:bigint("account_id",{mode:"number"}).notNull().references(()=>commercialAccounts.id,{onDelete:"cascade"}),clientKey:varchar("client_key",{length:96}).notNull().unique(),name:varchar("name",{length:160}).notNull(),purpose:varchar("purpose",{length:400}).notNull(),scopes:jsonb("scopes").notNull(),keyHash:varchar("key_hash",{length:128}).notNull().unique(),status:portfolioReviewStatusEnum("status").notNull().default("draft"),createdBy:integer("created_by").notNull().references(()=>users.id),rotatedAt:timestamp("rotated_at",{withTimezone:true}),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow()},(t)=>({accountIdx:index("api_client_account_idx").on(t.accountId,t.status)}));
export const propertyApiUsage=pgTable("property_api_usage",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),clientId:bigint("client_id",{mode:"number"}).notNull().references(()=>propertyApiClients.id,{onDelete:"cascade"}),requestKey:varchar("request_key",{length:96}).notNull().unique(),endpoint:varchar("endpoint",{length:160}).notNull(),scope:varchar("scope",{length:96}).notNull(),responseCount:integer("response_count").notNull(),purposeReference:varchar("purpose_reference",{length:160}).notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()});
export const planningAggregateReports=pgTable("planning_aggregate_reports",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),reportKey:varchar("report_key",{length:96}).notNull().unique(),accountId:bigint("account_id",{mode:"number"}).notNull().references(()=>commercialAccounts.id,{onDelete:"cascade"}),name:varchar("name",{length:200}).notNull(),cohortSize:integer("cohort_size").notNull(),minimumCohort:integer("minimum_cohort").notNull(),status:portfolioReviewStatusEnum("status").notNull().default("draft"),suppressed:boolean("suppressed").notNull().default(false),summary:jsonb("summary").notNull(),createdBy:integer("created_by").notNull().references(()=>users.id),publishedBy:integer("published_by").references(()=>users.id),publishedAt:timestamp("published_at",{withTimezone:true}),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow()},(t)=>({accountIdx:index("planning_report_account_idx").on(t.accountId,t.status)}));
export const ruralServiceCases=pgTable("rural_service_cases",{id:bigint("id",{mode:"number"}).primaryKey().generatedAlwaysAsIdentity(),caseKey:varchar("case_key",{length:96}).notNull().unique(),accountId:bigint("account_id",{mode:"number"}).notNull().references(()=>commercialAccounts.id,{onDelete:"cascade"}),parcelId:integer("parcel_id").references(()=>parcels.id),programType:varchar("program_type",{length:96}).notNull(),status:portfolioReviewStatusEnum("status").notNull().default("draft"),consentReference:varchar("consent_reference",{length:160}).notNull(),serviceReference:varchar("service_reference",{length:160}).notNull(),createdBy:integer("created_by").notNull().references(()=>users.id),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true}).notNull().defaultNow()},(t)=>({accountIdx:index("rural_case_account_idx").on(t.accountId,t.status)}));
